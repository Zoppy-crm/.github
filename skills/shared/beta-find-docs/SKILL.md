...
Prefer using the `ctx7` command directly (globally installed). Only fall back to `npx` if `ctx7` is not found.

Before running any `ctx7` command, check if it is available:
