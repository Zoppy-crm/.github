...
3. **Backend proof of life**: query the ingest store for the last 10 minutes
   (e.g. Pyroscope `QuerierService/LabelValues {name:"service_name",
   start:now-10m}`; Loki count for the new log line). Dashboards lie; recent
   ingest doesn't.
...
## 5. Session/transcript forensics (meta)

`~/.claude/projects/<dir>/<session>.jsonl` and `~/.claude/file-history/` are
queryable ground truth for "what actually happened in a past session" — per-hour
entry histograms find gaps; `grep -rl <artifact-id> ~/.claude/projects` finds
where something was really created.
