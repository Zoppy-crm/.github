...
| `src/cross_cutting/helpers/<topic>.helper.py` *(created on first real need)* | Business-concept logic shared by **2+ features** that doesn't fit `utils/`. Stateless, no I/O. | Single-feature use (move to feature helpers); pure technical utilities (move to `utils/`). |
